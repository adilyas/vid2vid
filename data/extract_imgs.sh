...
for video_path in `ls -v $SOURCE_DIR/*.MOV`;
do
    video_filename=$(basename $video_path)
    video_name="${video_filename%.*}"
    ffmpeg_dir=$TMP_DIR/$video_name

    echo "Creating ffmpeg output directory $ffmpeg_dir for $video_filename if not exists"
    mkdir -p $ffmpeg_dir

    if ! [ -f "$ffmpeg_dir/00001.jpg" ]
    then
        echo "Run ffmpeg for $video_filename"
        ffmpeg -v warning -ss 0 -i $video_path -to $MAX_LEN -filter:v fps=$FPS $ffmpeg_dir/%05d.jpg
    fi
done;
